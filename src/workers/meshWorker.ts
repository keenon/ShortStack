...
function generateProceduralTool(
    ...
    // --- 0. LOCAL MEMORY MANAGEMENT ---
    // Fixes "deleted object" errors by tracking intermediates created in this scope
    const localGarbage: any[] = [];
    const collectLocal = <T>(obj: T): T => { if(obj && (obj as any).delete) localGarbage.push(obj); return obj; };

    // --- 1. PROPORTIONAL SCALING & STEP GENERATION (Hoisted for Scope) ---
    // Defined outside try/catch so fallback can access 'steps'
    ...
    try {
        ...
        if (parts.length > 0) {
            ...
            return { 
                manifold: finalAligned, 
                vertProperties: fallbackMesh.vertProperties, 
                triVerts: fallbackMesh.triVerts 
            };
        }
